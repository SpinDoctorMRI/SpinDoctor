function results = solve_btpde_midpoint_flow(femesh, velocity, tautype, hktype, setup, savepath, save_magnetization)
...
%
%   femesh: struct
%   velocity: cell, {ncompartment}[3 x nelement]
%   tautype: int, stabilization parameters. Implemented four types.
%   hktype: int, diameter of elements. Implemented two types.
%   setup: struct
...
do_save = nargin >= nargin(@solve_btpde_midpoint_flow) - 1;

% Provide default value
if nargin < nargin(@solve_btpde_midpoint_flow)
...
% damping matrix
D_cmpts = cell(1, ncompartment);

hk_cmpts = cell(1, ncompartment);
norm2_v_cmpts = cell(1, ncompartment);

% SUPG matrix
vv_cmpts= cell(1, ncompartment);
mv_cmpts= cell(1, ncompartment);
Jxx_cmpts= cell(1, ncompartment);
Jy_cmpts= cell(1, ncompartment);
Jz_cmpts= cell(1, ncompartment);
...
for icmpt = 1:ncompartment
    ...
    % damping matrix
    D_cmpts{icmpt} = damping_matrix_3D(elements', points', volumes', velocity{icmpt});
    K_cmpts{icmpt} = K_cmpts{icmpt}+D_cmpts{icmpt};
    ...
    % truncate
    truncate_centre = -setup.geometry.height/2+4;%1;

    % exp
    truncate = exp(-(points(3,:) - truncate_centre).^2)';
    rho_cmpts{icmpt} = rho_cmpts{icmpt} .* truncate;

    if tautype ~=0
        if hktype ==1
            hk_cmpts{icmpt} = computeDiameters_dir(points',elements',[0,0,1]);
        elseif hktype==2
            hk_cmpts{icmpt} = computeDiameters_undir(points',elements');
        end
    
        norm2_v_cmpts{icmpt} = vecnorm(velocity{icmpt});
        norm2_v_cmpts{icmpt} = norm2_v_cmpts{icmpt}.';
        % prepare SUPG matrix
        [vv_cmpts{icmpt},...
            mv_cmpts{icmpt},...
            Jxx_cmpts{icmpt},...
            Jy_cmpts{icmpt},...
            Jz_cmpts{icmpt}] =stabilized_matrix_3D(elements',points',volumes', velocity{icmpt});
    end
end
        ...
        for iint = 1:ninterval
            ...
            if tautype==0
                ...
                A_stable] = SUPG_matrix(vv_cmpts,...
                                        ...
                                        femesh.elements,...
                                        tautype);
                M_plus_M_stable = M + dt * theta * M_stable;
                % Right hand side Jacobian
                J = -( (K+dt * theta*C_stable) + Q + R + 1i * seq.call(interval_midpoint) * q * (A+ dt * theta* A_stable) );
            end

            
            % Factorize left hand side matrix
            [L, U, P, QQ, D] = lu(M_plus_M_stable - dt * theta * J);

            % Right hand side matrix
            E = M_plus_M_stable + dt * (1 - theta) * J;
            ...
            if tautype==0
                ...
                A_stable] = SUPG_matrix(vv_cmpts,...
                                        ...
                                        femesh.elements,...
                                        tautype);
                M_plus_M_stable = M + dt_last * theta * M_stable;
                % Right hand side Jacobian
                J = -( (K+ dt_last * theta*C_stable) + Q + R + 1i * seq.call(interval_midpoint) * q * (A+ dt_last * theta* A_stable) );
            
            end
            
            
            E = M_plus_M_stable + dt_last * (1 - theta) * J;
            [L, U, P, QQ, D] = lu(M_plus_M_stable - dt_last * theta * J);
            y = QQ * (U \ (L \ (P * (D \ (E * y)))));
            t = t + dt_last;
        end
