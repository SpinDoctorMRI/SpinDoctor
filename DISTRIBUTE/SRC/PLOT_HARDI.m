...
ADC_Ellipsoid = trisurf(C_sph,ADC_alldir(:,1),ADC_alldir(:,2),ADC_alldir(:,3),ADC_interp,...
    'facealpha',0.9); view(3); 

view([2,2,1]); caxis([0,1]); 
colorbar; axis equal;
...
h = plot3(ADC_allcmpts_alldir.*points(:,1),ADC_allcmpts_alldir.*points(:,2),...
    ADC_allcmpts_alldir.*points(:,3),'k.','markersize',5);