...
%% Mesh file
filename = "mesh_files/cylinders/1axons_multilayer";
...
params_cells.("height") = 5;                            % Cylinder height
...
params_cells.("refinement")  = 0.2;                     % Tetgen refinement parameter

%% PDE parameters
params_domain.("diffusivity_in") = 0.002;               % Diffusion coefficient IN (scalar or tensor)
params_domain.("diffusivity_out") = 0.002;              % Diffusion coefficient OUT (scalar or tensor)
...
experiment.("values") = [0:100:500 1000:1000:4000];     % g-, q-, or b-values [1 x namplitude]
experiment.("values_type") = "b";         	            % Type of values; "g", "q" or "b"
experiment.("sequences"){1} = PGSE(2500, 5000);         % Gradient sequences {1 x nsequence}
experiment.("sequences"){2} = PGSE(2500, 10000);        % Gradient sequences {1 x nsequence}
