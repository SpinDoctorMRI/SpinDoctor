function driver_get_PGSE_errors(meshname,segment_cell,direc) 
lsval = [1.0];
hval = [0.5];
...
fig_cell = figure(1);grid on;xlabel('b-values');ylabel('Relative error');legend('Location','eastoutside');
saveas(fig_cell,sprintf('%s/%s_test_pgse_cell.fig',direc,cellname))

if segment_cell==1
    fig_soma = figure(2);grid on;xlabel('b-values');ylabel('Relative error');legend('Location','eastoutside');
    saveas(fig_soma,sprintf('%s/%s_test_pgse_soma.fig',direc,cellname))

    for k = 1:ndendrites
        fig_dend = figure(2+k);grid on;xlabel('b-values');ylabel('Relative error');legend('Location','eastoutside');
        saveas(fig_dend,sprintf('%s/%s_test_pgse_dendrite_%d.fig',direc,cellname,k))
    end
end