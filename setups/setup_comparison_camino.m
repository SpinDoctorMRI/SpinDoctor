...
setup.geometry.tetgen_options = "-pq1.1a1.0VCn";              % Tetgen options (priority is inferior to refinement)
...
%% Gradient sequences
seq = read_scheme('camino_sequences/alex_scheme_camino.scheme');
if isfile('camino_sequences/alex_scheme.txt')
b_tensors = read_b_tensors('camino_sequences/alex_scheme.txt');
for i = 1:length(seq)
...
setup.mf.eigs.sigma = 1e-8;                            % Rerun simulation with or without saved results
