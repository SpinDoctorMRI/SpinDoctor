%DEFINE_PARAMS_2AXONS_DEFORM SpinDoctor input file.
...
%% Mesh file
filename = "mesh_files/cylinders/2axons_deform";
...
params_cells.("ncell") = 2;                             % Number of cells
...
params_cells.("deformation") = [0.05; 4/3*pi];          % Domain deformation; [a_bend, a_twist]
...
experiment.("values") = [50 100 1000];                  % g-, q-, or b-values [1 x namplitude]
experiment.("values_type") = "b";                       % Type of values; "g", "q" or "b"
experiment.("sequences"){1} = PGSE(2500, 10000);        % Gradient sequences {1 x nsequence}
% experiment.("sequences"){2} = CosOGSE(2500, 5000, 5); % Gradient sequences {1 x nsequence}
...
experiment.("mf").("neig_max") = 400;                   % Requested number of eigenvalues
