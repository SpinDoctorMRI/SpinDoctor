...
%% Gradient sequences
seq = read_scheme('custom_sequence_files/new_scheme.scheme');
if isfile('custom_sequence_files/new_scheme.txt')
b_tensors = read_b_tensors('custom_sequence_files/new_scheme.txt');
for i = 1:length(seq)
    seq{i}.b_tensor = b_tensors(:,i);
end
end
setup.gradient.sequences =seq;
setup.gradient.sequences{length(seq)+1} = PGSE(1000,4300);
setup.gradient.directions = unitsphere(120);
setup.gradient.values = [2500, 4000, 6000 ,7000,8000, 10000, 12000, 20000]; 
...
setup.mf.ninterval = 101;                               % Number of intervals to discretize time profile in MF (if not PGSE and doublePGSE)
