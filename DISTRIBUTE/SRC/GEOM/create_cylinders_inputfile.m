...
function [Rvec] = create_cylinders_inputfile(ncyl,Rmin,Rmax,dmin,dmax,fname,Hmax)
...
%Rvec = linspace(Rmin,Rmax,ncyl);

positions = (rand(ncyl*100000,2)-0.5)*Rmean*max(10,sqrt(ncyl))*40;
...
for icyl = 1:ncyl 
	...
	nslice{icyl} = 2*max(1,round(Hmax));
