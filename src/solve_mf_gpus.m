function results = solve_mf_gpus(femesh, setup, lap_eig, savepath, save_magnetization)
%SOLVE_MF_GPUS Compute the solution to the BTPDE using Matrix Formalism on GPU.
%
%   SOLVE_MF_GPUS(FEMESH, SETUP, LAP_EIG) solves the BTPDE using
%   Matrix Formalism and returns results.
%
%   SOLVE_MF_GPUS(FEMESH, SETUP, LAP_EIG, SAVEPATH) saves the results of each iteration at
...
%
%   SOLVE_MF_GPUS(FEMESH, SETUP, LAP_EIG, SAVEPATH, SAVE_MAGNETIZATION) also omits saving
...
do_save = nargin >= nargin(@solve_mf_gpus) - 1;
...
if nargin < nargin(@solve_mf_gpus)
