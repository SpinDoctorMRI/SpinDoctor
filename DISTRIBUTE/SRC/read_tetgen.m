...
if (Ncmpt_tmp ~= Ncmpt)
    ...
    if (Nboundary_tmp ~= Nboundary)
        ...
        for icmpt = 1:Ncmpt
        % for icmpt = Ncmpt:Ncmpt
%             icmpt
%             length(Pts_ind{icmpt})
            Ele_cmpt_reorder{icmpt} = Ele_cmpt{icmpt};            
            oldcode = Pts_ind{icmpt};
            newcode = 1:length(Pts_ind{icmpt});
            assert(numel(newcode) == numel(oldcode), ...
                'newcode and oldecode must have the same number of elements');
            [toreplace,bywhat] = ismember(Ele_cmpt_reorder{icmpt},oldcode);
            Ele_cmpt_reorder{icmpt}(toreplace) = newcode(bywhat(toreplace));
%             disp('here 2');
%             mattemp = Ele_cmpt_reorder{icmpt};
%             Ele_cmpt_reorder{icmpt} = Ele_cmpt{icmpt};
...
%             end
%             disp('error')
%             max(max(abs(Ele_cmpt_reorder{icmpt}-mattemp)))
            ...
            for iboundary = 1:Nboundary
                ...
                if (onbd == 1)
                    ...
                    assert(numel(newcode) == numel(oldcode), ...
                        'newcode and oldecode must have the same number of elements');
                    [toreplace,bywhat] = ismember(Fac_boundary_reorder{icmpt}{iboundary},oldcode);
                    Fac_boundary_reorder{icmpt}{iboundary}(toreplace) = newcode(bywhat(toreplace));
%                     mattemp = Fac_boundary_reorder{icmpt}{iboundary};
%                     Fac_boundary_reorder{icmpt}{iboundary} = Fac_boundary{iboundary};
%                     for ii = 1:length(Pts_ind{icmpt})
...
%                     end
%                     disp('error 2')
%                     max(max(abs(Fac_boundary_reorder{icmpt}{iboundary}-mattemp)))
                    
                    Pts_boundary_reorder{icmpt}{iboundary} = Pts_boundary{iboundary};
                    [toreplace,bywhat] = ismember(Pts_boundary_reorder{icmpt}{iboundary},oldcode);
                    Pts_boundary_reorder{icmpt}{iboundary}(toreplace) = newcode(bywhat(toreplace));
%                     mattemp = Pts_boundary_reorder{icmpt}{iboundary};
%                     
%                     Pts_boundary_reorder{icmpt}{iboundary} = Pts_boundary{iboundary};
%                     for ii = 1:length(Pts_ind{icmpt})
...
%                     end
%                     disp('error 3')
%                     max(max(abs(Pts_boundary_reorder{icmpt}{iboundary}-mattemp)))
                else
