%DEFINE_PARAMS_1SPHERE_MULTILAYER SpinDoctor input file.
...
%% Meshfile
filename = "mesh_files/spheres/1sphere_multilayer";
...
params_cells.("rmin") = 10;                             % Minimum radius
...
params_domain.("permeability_in_out") = 5e-4;           % Permeability IN-OUT interface
params_domain.("permeability_out_ecs") = 5e-4;          % Permeability OUT-ECS interface
...
experiment.("flat_dirs") = false;                       % Choose between 3d or 2d distributed directions
experiment.("remove_opposite") = false;                 % Choose whether to not compute opposite directions
experiment.("direction") = [1.0; 1.0; 1.0];             % Gradient direction; [g1; g2; g3] (ignored if ndir>1)
experiment.("values") = [0 100 500 1000 2000 3000 4000];% g-, q-, or b-values [1 x namplitude]
experiment.("values_type") = "b";                       % Type of values; "g", "q" or "b" or "g"
...
% experiment.("sequences"){1} = SinOGSE(2500, 10000, 5);   % Gradient sequences {1 x nsequence}
...
experiment.("mf").("neig_max") = 200;                   % Requested number of eigenvalues
