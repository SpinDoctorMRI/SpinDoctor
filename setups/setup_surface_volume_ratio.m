...
%% Gradient sequences
setup.gradient.sequences = {PGSE(1000,1000),PGSE(1000,1500),PGSE(1000,2000)};
% setup.gradient.sequences = {PGSE(1000,1000),PGSE(1000,1500),PGSE(1000,2000),PGSE(6000,26000),PGSE(6000,52000),PGSE(6000,62000),PGSE(6000,82000),PGSE(6000,92000),PGSE(6000,104000),PGSE(6000,208000),PGSE(6000,720500)};

setup.gradient.directions = [1;0;0];
setup.gradient.values = [1000 1500 2500 4000 6000];% 7000 8000 10000 20000 40000 60000];
% setup.gradient.values = [ 20000 40000 60000];
...
% setup.btpde.reltol = 1e-3;                              % Relative tolerance for ODE solver
% setup.btpde.abstol = 1e-5;                              % Absolute tolerance for ODE solver
