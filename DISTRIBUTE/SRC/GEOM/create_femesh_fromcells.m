function [fname_tetgen_femesh] = ...
   create_femesh_fromcells(params_cells,fname_cells,...
   params_domain_geom,params_domain_femesh,fname_tetgen)
...
elseif (cell_shape == 3)
    ...
    if (exist([fname_cells, '_elements.txt'], 'file') ...
            && exist([fname_cells, '_nodes.txt'], 'file')) == 0        
        gmsh_to_fem_Try(fname_cells);        
    end
    disp(['Reading from neuron FE mesh from ', fname_cells]);
    
    elements = load([fname_cells, '_elements.txt']);
    nodes = load([fname_cells, '_nodes.txt']);
    
    [VOL,EleV,EleC] = get_volume_mesh(nodes',elements');    
    centermass = ((EleC*EleV')/VOL);
    ...
    bb=faces2nodes(bindex, :);    %nodes of boundary facets
    aa=sort(bb, 2);
    facets=unique(aa, 'rows');
    facets = facets';
    
    ncell = 1;
    facets_cell{1} = facets;
    nodes_cell{1} = nodes;
    facets_labels_cell{1} =  ones(1,size(facets_cell{1},2));
    pt_in_cell = zeros(1,3);
    pt_in_cell(1,1:3) = centermass';
end
