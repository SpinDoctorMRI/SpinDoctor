%DEFINE_PARAMS_5AXONS_MYELIN_RELAX SpinDoctor input file.
...
%% Meshfile
filename = "mesh_files/cylinders/5axons_myelin_relax";
...
params_cells.("ncell") = 5;                             % Number of cells
params_cells.("rmin") = 3;                              % Minimum radius of cells
params_cells.("rmax") = 4;                              % Minimum radius of cells
params_cells.("dmin") = 0.1;                            % Minimum distance between cells (times mean(rmin,rmax))
params_cells.("dmax") = 0.2;                            % Maximum distance between cells (times mean(rmin,rmax))
params_cells.("height") = 10;                           % Cylinder height
...
params_cells.("in_ratio") = 0.5;                        % Ratio Rin/R, within range [0,0.99]
params_cells.("ecs_shape") = "tight_wrap";              % Shape of ECS: "no_ecs", "box", "convex_hull", or "tight_wrap".
params_cells.("ecs_ratio") = 0.3;                       % ECS gap; percentage in side length
...
params_domain.("relaxation_out") = 20e+03;              % T2-relaxation OUT. No relaxation: Inf
params_domain.("relaxation_ecs") = 80e+03;              % T2-relaxtion ECS. No relaxation: Inf
...
params_domain.("permeability_in_out") = 1e-03;          % Permeability IN-OUT interface
params_domain.("permeability_out_ecs") = 1e-03;         % Permeability OUT-ECS interface
...
experiment.("values") = [2 10] * 1e-05;                 % g-, q-, or b-values [1 x namplitude]
experiment.("values_type") = "q";                       % Type of values; "g", "q" or "b"
experiment.("sequences"){1} = PGSE(5000, 5000);         % Gradient sequences {1 x nsequence}
experiment.("sequences"){2} = PGSE(5000, 10000);        % Gradient sequences {1 x nsequence}
experiment.("sequences"){3} = PGSE(10000, 20000);       % Gradient sequences {1 x nsequence}
...
experiment.("mf").("neig_max") = Inf;                   % Requested number of eigenvalues
experiment.("mf").("ninterval") = 10;                   % Number of intervals to discretize time profile in MF (if not PGSE)
